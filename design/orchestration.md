...
- **Permission mode for worker sessions** (raised during implementation): agents need tool access (bash, file edit) to do real work. The default `acceptEdits` only allows file edits. Should `coven worker` default to a more permissive mode like `bypassPermissions`, or require the user to pass `--permission-mode` via extra args?
  ...
  Analysis from code review: all agents except dispatch need Bash access. Plan needs `mv`/`git mv` to move issues between `issues/` and `review/`, plus `git add`/`git commit`. Implement needs `cargo test`/`clippy`/`fmt` plus git operations. Audit needs git operations. Only dispatch can work without Bash — it just reads files and outputs a decision. Since workers run unattended (no human at the keyboard), any permission prompt blocks indefinitely, which rules out `default`, `acceptEdits`, and `plan` modes for agents that need Bash. Options:

  (a) Default all worker sessions to `bypassPermissions`. Simple, works, but gives agents more power than they need.
  (b) Per-agent `permission_mode` in frontmatter. Dispatch uses `acceptEdits`, others use `bypassPermissions`. More granular but adds complexity to the agent format.
  (c) Use `.claude/settings.json` `allowedTools` to whitelist specific bash patterns, then use `acceptEdits`. Most secure but requires maintaining a command allowlist per project.

  Recommendation (claude): start with (a) — the agents are controlled by prompt files the human writes, so trust is already established. Revisit if finer control is needed.
