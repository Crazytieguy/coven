...
- **CLAUDE.md integration**: `coven init` currently prints a suggestion to add a `.coven/workflow.md` reference to CLAUDE.md. Should init update CLAUDE.md automatically instead? Concerns: (a) CLAUDE.md may not exist yet, (b) modifying an existing CLAUDE.md requires careful insertion rather than overwriting, (c) the user may prefer to write their own reference. Current approach (print suggestion) is safe but easy to miss.
...
- **Orphaned worktree cleanup**: `coven gc` command. Lists all git worktrees, compares against live worker state, removes non-main worktrees that no live worker owns (worktree + branch). Worker state files are already cleaned by PID liveness check in `read_all`; gc handles the worktrees themselves. (Implemented in `commands/gc.rs` and `worktree::list_worktrees`.)
- **Workflow documentation**: `coven init` creates `.coven/workflow.md` alongside agent templates. Chose option (a) from the design discussion — separate file in the `.coven/` namespace rather than replacing the project-root `workflow.md`. Documents the issue file format, states, lifecycle, and directory structure. Init prints a suggestion to reference it from CLAUDE.md. (Implemented in `commands/init.rs`.)
